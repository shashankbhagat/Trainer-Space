...
namespace AngelHack.DataLayer
{
    public class RepositoryMain
    {
        ...
        public static List<MainVM> getSpacewithAvailibility(string location, string studioType)
        {
            ...
            try
            {
                string sql = "select s.Title as Title,s.ImageUrl as ImageUrl,case when isnull(b.Id,0)=0 then 0 else 1 end as Available from Space s inner join Location l on (s.LocationId=l.Id) inner join SpaceType sp on(sp.Id = s.SpaceTypeId) left join Booking b on(s.Id = b.SpaceId) where l.Name = @location and sp.Title = @studioType";
